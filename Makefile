...
test-e2e: generate fmt vet ## Run end-to-end tests
	USE_EXISTING_CLUSTER=true NAME_PREFIX=$(NAME_PREFIX) go test -v ./test/e2e -coverprofile cover.out -ginkgo.v -ginkgo.progress -namespace "$(NAMESPACE)" -eventually-timeout 8m -timeout 30m -delete-timeout 8m $(GO_TEST_FLAGS)
