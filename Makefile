...
test-e2e: generate fmt vet ## Run end-to-end tests
	USE_EXISTING_CLUSTER=true NAME_PREFIX=$(NAME_PREFIX) go test -v ./test/e2e -ginkgo.label-filter="slow || fast" -ginkgo.v -coverprofile cover.out -namespace "$(NAMESPACE)" -eventually-timeout 8m -timeout 40m -delete-timeout 8m $(GO_TEST_FLAGS)
test-ph: generate fmt vet ## Run phone-home tests
    USE_EXISTING_CLUSTER=true NAME_PREFIX=$(NAME_PREFIX) go test -v ./test/ph -v -ginkgo.label-filter="slow || fast" -ginkgo.v -coverprofile cover.out -namespace "$(NAMESPACE)" -eventually-timeout 8m -timeout 40m -delete-timeout 8m $(GO_TEST_FLAGS)
##@ Build
