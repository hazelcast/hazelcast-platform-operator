...
# hazelcast.com/hazelcast-enterprise-operator-bundle:$VERSION and hazelcast.com/hazelcast-enterprise-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= hazelcast.com/hazelcast-enterprise-operator
...
# Default namespace
NAMESPACE ?= hazelcast-enterprise-operator-system
...
test-e2e: generate fmt vet ## Run end-to-end tests
	USE_EXISTING_CLUSTER=true go test -v ./test/e2e -coverprofile cover.out -namespace $(NAMESPACE) -test-timeout 15m -delete-timeout 15m
