	...
	It("should upgrade HZ version after pause/resume with default partition count during 4 hours and keep 45 GB data", Serial, Tag(EE|AnyCloud), func() {
		...
		for cycle := 1; cycle <= totalPauseResumeCycles; cycle++ {
			...
			for i := 0; i < numMaps; i++ {
				...
				WaitForMapSize(hzLookupKey, m.MapName(), int(float64(cycle*totalFillRepeats*mapSizeInMb)*128), 2*Minute)
			}
		}
		...
		for i := 0; i < numMaps; i++ {
			m := hazelcastconfig.DefaultMap(types.NamespacedName{Name: fmt.Sprintf("map-%d-%s", i, hazelcast.Name), Namespace: hazelcast.Namespace}, hazelcast.Name, labels)
			m.Spec.HazelcastResourceName = hazelcast.Name
			WaitForMapSize(hzLookupKey, m.MapName(), int(float64(totalPauseResumeCycles*totalFillRepeats*mapSizeInMb)*128), 2*Minute)
		}
	})
})
