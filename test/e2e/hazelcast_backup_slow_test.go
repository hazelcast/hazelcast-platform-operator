...
var _ = Describe("Hazelcast Backup", Label("backup_slow"), func() {
	...
	It("should restart successfully with Cache after shutting down Hazelcast", Label("slow"), func() {
		if !ee {
			Skip("This test will only run in EE configuration")
		}
		setLabelAndCRName("hpch-1")
		ctx := context.Background()
		baseDir := "/data/hot-restart"
		...
		c.Spec.PersistenceEnabled = true
		Expect(k8sClient.Create(context.Background(), c)).Should(Succeed())
		assertDataStructureStatus(chLookupKey, hazelcastcomv1alpha1.DataStructureSuccess, c)

		// TODO ---->
		FillTheMapData(ctx, hzLookupKey, true, c.Name, 100)
		...
		WaitForMapSize(context.Background(), hzLookupKey, c.Name, 100, 30*Minute)
	})
})
