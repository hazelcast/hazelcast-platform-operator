...
var _ = Describe("Hazelcast Cache Config with Persistence", Label("cache_persistence"), func() {
	...
	AfterEach(func() {
		...
		DeleteAllOf(&hazelcastv1beta1.Cache{}, &hazelcastv1beta1.CacheList{}, hzNamespace, labels)
	...
	It("should fail when persistence of Cache CR and Hazelcast CR do not match", Label("fast"), func() {
		...
		assertDataStructureStatus(chLookupKey, hazelcastv1beta1.DataStructureFailed, m)
	...
	It("should keep the entries after a Hot Backup", Label("slow"), func() {
		...
		assertDataStructureStatus(chLookupKey, hazelcastv1beta1.DataStructureSuccess, cache)
	...
	It("should persist the cache successfully created configs into the config", Label("fast"), func() {
		...
		for _, cache := range caches {
			...
			if cache == "cachefail" {
				assertDataStructureStatus(types.NamespacedName{Name: c.Name, Namespace: c.Namespace}, hazelcastv1beta1.DataStructureFailed, c)
				continue
			}
			assertDataStructureStatus(types.NamespacedName{Name: c.Name, Namespace: c.Namespace}, hazelcastv1beta1.DataStructureSuccess, c)
		}
		...
		Expect(k8sClient.Delete(context.Background(),
			&hazelcastv1beta1.Cache{ObjectMeta: v1.ObjectMeta{Name: "cache2", Namespace: hazelcast.Namespace}})).Should(Succeed())
