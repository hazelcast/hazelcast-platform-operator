...
local_resource(
  'go-compile',
  'make build-tilt',
  deps=['./main.go','api/','controllers/','pkg/',],
...
cmd_button('Delete CRs and PVCs',
            argv=['sh', '-c', '(kubectl delete $(kubectl get hazelcast,managementcenter,hotbackup,map -o name) 2> /dev/null || echo "No CRs" ) && kubectl delete pvc -l "app.kubernetes.io/managed-by=hazelcast-platform-operator"'],
            resource='hazelcast-platform-controller-manager',
            location=location.RESOURCE,
            icon_name='delete',
